...
//   javac -cp multiverse-core-0.7.0.jar TestStmQueues.java
//   java -cp multiverse-core-0.7.0.jar;. TestStmQueues
...
public class TestStmQueues extends Tests {
  ...
  private static <T> T takeOne(BoundedQueue<T> bq1, BoundedQueue<T> bq2) throws Exception {
      return atomic( () -> {
              if( !bq1.isEmpty() ) return bq1.take();
              if( !bq2.isEmpty() ) return bq2.take();
              retry();
              return null;
          } );
  }
  ...
  // (which I cannot get to work, though), but it should look like this:
  // private static <T> T takeOne(BoundedQueue<T> bq1, BoundedQueue<T> bq2) throws Exception {
  //     return assert(() -> bq1.take(), () -> bq2.take());
  // }

  // internally, it is implemented something like this:
