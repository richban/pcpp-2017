...
class FunList<T> {
  ...
  protected static <T> Node<T> remove(Node<T> elem, T x) {
    ...
    Node<T> NextNode = remove(elem.next, x);
    if (elem.item == x) {
      return NextNode;
    }

    if (NextNode != elem.next) {
      return new Node<T>(elem.item, NextNode);
    }
    return elem;
  }

  public Funlist<T> remove(T x) {
    Node<T> newNode = remove(first, x);
    if (fist != newNode) {
      return new FunList<T>(newNode);
    }
    return this;
  }
  ...
  public FunList<T> filter(Predicate<T> p) {
    return new FunList<T> (reduce(null, (nodes, elem) -> {
      return p.test(elem) ? append(nodes, new Node<T>(elem, null)) : nodes;
    }));
  }
